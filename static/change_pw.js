...
async function changePassword() {
    ...
    resultDiv.innerHTML = '<p style="color:blue;">비밀번호를 변경하는 중...</p>';

    try {
        // ✅ 비밀번호 업데이트 (row 제거, current_password 추가)
        const updateRes = await fetch('/update_password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                current_password: currentPassword,  // ✅ 필드명 수정
                new_password: newPassword           // ✅ 필드명 수정
            })
        });
